...
\title{Black Hole Lifecycle: Field-Driven Cosmology with Relics as Dark Matter}
...
\begin{abstract}
This paper extends the Unified Scalar Time-Gradient Field Theory from micro-scale decay anomalies to cosmic-scale cosmology. The same scalar field $G(t)$ that explains nuclear decay-rate variations replaces dark energy, while black hole end-state relics replace cold dark matter. We numerically solve the Friedmann equation with scalar field dynamics, black hole mass evolution through evaporation to relic freeze-out, relic identity stability, linear perturbation growth, and observable predictions including rotation curves and gravitational lensing. All seven theory closure checks pass, demonstrating that a single field framework can simultaneously address anomalies across nuclear, gravitational, and cosmological domains.
\end{abstract}

\section{Cosmic Expansion from Field (Replaces Dark Energy)}
...
\section{Relics as Dark Matter (Fluid Description)}
...
The scalar field $G(t)$ simultaneously addresses micro-scale decay anomalies, drives cosmic expansion (replacing dark energy), and produces the dark matter component via black hole relic end-states. One field, two cosmological jobs, no fine-tuning between scales.
