...
field \$G(t)\$ (the mathematical expression of the axiom).
...
\begin{figure}
\centering
\includegraphics{fig-multi-scale-field}
...
\toprule\noalign{}
Sector & \$X\_\{\textbackslash text\{eff\}\}\$ & \$c\_X\$ & Domain \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
Decay & \$\textbackslash lambda\_\{\textbackslash text\{eff\}\}\$ &
\$Q\_\textbackslash alpha d\_\textbackslash alpha + Q\_\textbackslash mu
d\_\textbackslash mu\$ & QM (nuclear) \\
Gravity & \$g\_\{\textbackslash text\{eff\}\}\$ & \$\$\$um
f\_i(d\_\textbackslash alpha + \textbackslash beta\_i
d\_\textbackslash mu)\$ & GR (macroscopic) \\
Inertia & \$a\_\{\textbackslash text\{eff\}\}\$ & same as time gradient
& GR (macroscopic) \\
Fine structure & \$\textbackslash alpha(\textbackslash phi)\$ &
\$d\_\textbackslash alpha\$ & QM + GR (fundamental) \\
Mass ratio & \$\textbackslash mu(\textbackslash phi)\$ &
\$d\_\textbackslash mu\$ & QM + GR (fundamental) \\
\end{longtable}
...
dilaton couplings (\$d\_\textbackslash alpha\$,
\$d\_\textbackslash mu\$). Two free parameters. All sectors. All scales.

\begin{figure}
\centering
\includegraphics{fig-correction-law-hub}
...
\tightlist
\item
  Drives expansion (\$H\^{}2 \$\$im
  \textbackslash rho\_\{\textbackslash text\{field\}\}\$)
\item
...
\item
  ...
  \textbackslash lambda\_0(1 + \textbackslash alpha G)\$)
\item
  Modifies the time gradient (\$g\_\{\textbackslash text\{eff\}\} =
  g\_0(1 + \$\$amma G)\$)
\item
  Prevents singularities (\$G \textbackslash to \$\$amma\^{}\{-1\}\$
  stops collapse)
\item
...
(\$dS/dt = 0\$). But coarse-graining (incomplete description) produces
\$dS/dt \textgreater{} 0\$. The entropy arrow is not a law. It is a
consequence of information preservation plus incomplete description. The
arrow of time = the direction of information resolution.
...
point of condensation. \$c\^{}2\$ is the field\textquotesingle s maximum
vibration speed --- the speed of light = the speed of information = the
...
One law: \$X\_\{\textbackslash text\{eff\}\} = X\_0(1 + c\_X G)\$.
