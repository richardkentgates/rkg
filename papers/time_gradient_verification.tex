...
central equation \$\textbackslash lambda\_\{\textbackslash text\{eff\}\}
= \textbackslash lambda\_0 {[}1 + \textbackslash alpha \textbackslash,
G(t){]}\$ correctly describes observed decay-rate anomalies, and that
the model\textquotesingle s variational principle produces correct
predictions for orbital mechanics, time dilation, and light bending
without invoking a force.
...
\item
  ...
  \$\textbackslash Phi(x,t)\$ across the universe. Mass and energy
  sources act as local retardants, creating a temporal topography where
  deep space is a high plateau and planetary bodies are valleys of slow
  time.
\item
  ...
  \$\textbackslash partial \textbackslash Phi / \textbackslash partial
  x\$.
\item
  \textbf{The Efficiency Postulate:} The constant \$c\$ is the speed
  limit of temporal efficiency. Light follows the path that maximizes
  elapsed proper time (Fermat\textquotesingle s principle).
\end{itemize}
...
\$\textbackslash alpha\$ is a coupling strength, and \$G(t)\$ is the
dimensionless time-gradient field. This report verifies the mathematical
consistency of this equation and its predictions against experimental
data.
...
This is mathematically consistent. For a time-independent field \$G\$,
the decay rate reduces to the standard exponential law with a modified
constant. For a time-varying field, the effective rate tracks the field
instantaneously.

\hypertarget{effective-half-life}{%
\subsubsection{3.2 Effective Half-Life}\label{effective-half-life}}

The half-life is related to the decay constant by \$T\_\{1/2\} =
\textbackslash ln 2 / \textbackslash lambda\$. Substituting the
...
For small \$\textbackslash alpha G(t)\$, the Taylor expansion gives:
...
This approximation is valid when \$\textbar\textbackslash alpha
G(t)\textbar{} \textbackslash ll 1\$. For the BNL data where
\$\textbackslash alpha G \$\$im 10\^{}\{-3\}\$, the error from this
approximation is of order \$10\^{}\{-6\}\$, which is negligible compared
...
For a periodic field \$G(t) = \textbackslash cos(2\textbackslash pi t /
T + \textbackslash phi)\$, the integral evaluates to:
...
\textbackslash left{[}
\$\$in\textbackslash left(\textbackslash frac\{2\textbackslash pi
t\}\{T\} + \textbackslash phi\textbackslash right) -
\$\$in(\textbackslash phi) \textbackslash right{]}\textbackslash{]}

The decay curve is therefore a modulated exponential, with the
modulation amplitude determined by \$\textbackslash alpha\$ and the
period determined by the field \$G(t)\$.
...
where the ratio
\$\^{}\{32\}\textbackslash text\{Si\}/\^{}\{36\}\textbackslash text\{Cl\}\$
...
\item
  ...
  7.9(3) \textbackslash times 10\^{}\{-4\}\$
\item
  Phase shift: \$35(2)\$ days from January 1
\item
  Pearson correlation with \$1/R\^{}2\$: \$r = 0.52\$ for \$N = 239\$
  data points
\item
  Formal probability of random correlation: \$p = 6 \textbackslash times
  10\^{}\{-18\}\$
\end{itemize}

Fitting the model
\$\textbackslash lambda\_\{\textbackslash text\{eff\}\} =
\textbackslash lambda\_0{[}1 + \textbackslash alpha G(t){]}\$ with
\$G(t) = \textbackslash cos(2\textbackslash pi
t/T\_\{\textbackslash text\{year\}\} + \textbackslash phi)\$ yields:
...
The coupling constant \$\textbackslash alpha\$ represents the fraction
of the time-gradient field that affects nuclear decay rates. This value
is consistent across the BNL dataset.

\hypertarget{the-ptb-ra-226-data}{%
\subsubsection{4.2 The PTB Ra-226 Data}\label{the-ptb-ra-226-data}}

Siegert et al. (1998) measured \$\^{}\{226\}\textbackslash text\{Ra\}\$
decay rates at the Physikalisch-Technische Bundesanstalt (PTB) in
Germany using an ionization chamber over 15 years (1983-1998). The data
showed:
...
\item
  Annual oscillation amplitude:
  \$\textbackslash delta\textbackslash lambda/\textbackslash lambda =
  8.3(2) \textbackslash times 10\^{}\{-4\}\$
\item
  Phase shift: \$59\$ days from January 1
\end{itemize}
...
The coupling constant for \$\^{}\{226\}\textbackslash text\{Ra\}\$ is
within 5\% of the \$\^{}\{32\}\textbackslash text\{Si\}\$ value,
...
Earth-Sun distance. The annual variation in \$1/R\^{}2\$ is:
...
\$10\^{}\{-2\}\$, consistent with the model\textquotesingle s prediction
of a weak but measurable coupling.
...
\$\^{}\{54\}\textbackslash text\{Mn\}\$ decay rates during the solar
flare of December 13, 2006. The model describes this as a Gaussian pulse
in the time-gradient field:

\textbackslash{[}G(t) = A\_\{\textbackslash text\{flare\}\}
\textbackslash, e\^{}\{-(t - t\_0)\^{}2 /
(2\$\$igma\^{}2)\}\textbackslash{]}

The fitted parameters gave \$\textbackslash alpha \textbackslash,
A\_\{\textbackslash text\{flare\}\} \textbackslash approx -2.5
\textbackslash times 10\^{}\{-3\}\$, with the onset preceding the
...
\tightlist
\item
  Annual frequency (\$1.00 \textbackslash,
  \textbackslash text\{yr\}\^{}\{-1\}\$) at \$\textgreater5\$\$igma\$
  significance
\item
  33-day frequency at \$\textgreater5\$\$igma\$ significance
\item
  12.5 year\$\^{}\{-1\}\$ frequency at \$\textgreater3\$\$igma\$
  significance
\end{itemize}

The 33-day period matches the solar core rotation rate. The 12.5
year\$\^{}\{-1\}\$ frequency matches solar r-mode oscillations. These
frequencies are consistent with the model\textquotesingle s prediction
that the time-gradient field is modulated by solar dynamics.
...
a spherical mass \$M\$, the time dilation is:

\textbackslash{[}\textbackslash frac\{d\textbackslash tau\}\{dt\} =
\$\$qrt\{1 - \textbackslash frac\{2GM\}\{rc\^{}2\}\}
...
gradient. For a clock at distance \$r\$ from a mass \$M\$:
...
At Earth\textquotesingle s surface (\$r = R\_\textbackslash oplus\$):
...
efficiency. For a light ray passing a mass \$M\$ at impact parameter
\$b\$, the deflection angle is:

\textbackslash{[}\textbackslash delta\textbackslash phi =
\textbackslash frac\{4GM\}\{c\^{}2 b\}\textbackslash{]}

For the Sun (\$M = M\_\textbackslash odot\$, \$b =
R\_\textbackslash odot\$):
...
\$\textbackslash pm\$ 0.05 arcseconds) and subsequent measurements. The
time-gradient model predicts the same light bending as general
relativity.
...
\begin{longtable}[]{@{}lll@{}}
\toprule\noalign{}
System & \$\textbackslash alpha\$ & Source \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
\$\^{}\{32\}\textbackslash text\{Si\}\$ (BNL) & \$7.9
\textbackslash times 10\^{}\{-4\}\$ & Jenkins 2008 \\
\$\^{}\{36\}\textbackslash text\{Cl\}\$ (BNL) & \$6.2
\textbackslash times 10\^{}\{-4\}\$ & Jenkins 2008 \\
\$\^{}\{226\}\textbackslash text\{Ra\}\$ (PTB) & \$8.3
\textbackslash times 10\^{}\{-4\}\$ & Siegert 1998 \\
\$\^{}\{54\}\textbackslash text\{Mn\}\$ (Purdue) & \$2.5
\textbackslash times 10\^{}\{-3\}\$ & Jenkins 2009 \\
\end{longtable}
...
The capture rate
\$\textbackslash Lambda\_\{\textbackslash text\{capture\}\}\$ scales
approximately as \$Z\^{}4\$ for nuclear muon capture. The time-gradient
model identifies this scaling as arising from the nuclear time gradient,
which scales with the nuclear binding energy per nucleon.
...
\$\textbar\textbackslash alpha G\textbar{} \textbackslash ll 1\$ \\
Decay curve integration & Calculus verification & Verified \\
BNL Si-32 anomaly fit & Parameter estimation & \$\textbackslash alpha =
7.9 \textbackslash times 10\^{}\{-4\}\$ \\
PTB Ra-226 anomaly fit & Parameter estimation & \$\textbackslash alpha =
8.3 \textbackslash times 10\^{}\{-4\}\$ \\
Solar flare transient & Gaussian fit & \$\textbackslash alpha
A\_\{\textbackslash text\{flare\}\} = -2.5 \textbackslash times
10\^{}\{-3\}\$ \\
...
Cross-system coherence & Parameter comparison & \$\textbackslash alpha\$
...
\$\textbackslash lambda\_\{\textbackslash text\{eff\}\} =
\textbackslash lambda\_0{[}1 + \textbackslash alpha G(t){]}\$ correctly
describes the observed decay-rate anomalies in Si-32, Cl-36, Ra-226, and
Mn-54. The model\textquotesingle s variational principle (maximum
temporal efficiency) produces correct predictions for orbital mechanics,
time dilation, and light bending without invoking a force. The coupling
constant \$\textbackslash alpha\$ is consistent across isotopes within
an order of magnitude, supporting the model\textquotesingle s prediction
of a universal time-gradient field with isotope-dependent coupling.
